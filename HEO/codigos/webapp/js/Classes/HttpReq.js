function HttpReq(metodoRecebido, urlRecebido, fSucesso, fErro) {
    ...
    this.enviar = function (variaveis, logado, telaCarregando, tempoEsgotado) {
                ...
                if (this.status == 401) {
                    ...
                    if (false) {
                        var httpReq = new HttpReq("post", pagina.getWs() + "usuario/entrar", function (res) {
                            ...
                            //pagina.erroPadraoConexao();,
                            console.log("erro padra de conexao");
                            usuario = new Usuario();
                        });
        ...
        if (this.metodo == "post") {
            ...
            this.xmlHttpRequest.send(variaveis + ((window.localStorage.getItem("idUnidade") != "undefined" && window.localStorage.getItem("idUnidade") > 0) ? ("&idUnidade=" + window.localStorage.getItem("idUnidade")) : ""));
            if (telaCarregando) {
